...
function About() {
  ...
  const [notifyWhenUpdateIsAvailable, setNotifyWhenUpdateIsAvailablePref] = useGlobalPref(
  ...
  return (
    <Setting>
      <Text>
        <Trans>
          <strong>Actual</strong> is a super fast privacy-focused app for managing your finances.
        </Trans>
      </Text>
      <View
        style={{
      ...
      >
        <Text>
          <Trans>Client version: {{ version: `v${window.Actual?.ACTUAL_VERSION}` }}</Trans>
        </Text>
        <Text>
        ...
        {notifyWhenUpdateIsAvailable && versionInfo?.isOutdated ? (
          <Link variant="external" to="https://actualbudget.org/docs/releases" linkColor="purple">
            <Trans>New version available: {versionInfo.latestVersion}</Trans>
          </Link>
        ) : (
          <Text style={{ color: theme.noticeText, fontWeight: 600 }}>
            {notifyWhenUpdateIsAvailable ? <Trans>You're up to date!</Trans> : null}
          </Text>
        )}
        <Text>
          <Link variant="external" to="https://actualbudget.org/docs/releases" linkColor="purple">
      ...
      <View>
        <Text style={{ display: "flex" }}>
          <Checkbox
            ...
            onChange={(e) => setNotifyWhenUpdateIsAvailablePref(e.currentTarget.checked)}
          />
...
function AdvancedAbout() {
  ...
  return (
    <Setting>
      <Text>
        <Trans>
          <strong>IDs</strong> are the names Actual uses to identify your budget internally. There
          are several different IDs associated with your budget. The Budget ID is used to identify
          your budget file. The Sync ID is used to access the budget on the server.
        </Trans>
      </Text>
      <Text>
        <Trans>
...
function ToolsNavigation() {
  ...
  return (
    <Setting>
      ...
      <View>
        <ToolsNavLink to="/schedules" icon={SvgCalendar3} label={t("Schedules")} />
        <ToolsNavLink to="/payees" icon={SvgStoreFront} label={t("Payees")} />
        <ToolsNavLink to="/rules" icon={SvgTuning} label={t("Rules")} />
        {isUsingServer && (
          <ToolsNavLink to="/bank-sync" icon={SvgCreditCard} label={t("Bank Sync")} />
        )}
