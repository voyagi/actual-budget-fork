...
test.describe("Enable Banking", () => {
  ...
  test("create account modal shows Enable Banking option", async () => {
    ...
    await expect(page.getByText(/Enable Banking provides PSD2-compliant access/)).toBeVisible();
  });
...
test.describe("Enable Banking - Configured", () => {
  ...
  test("shows privacy policy warning", async () => {
    await expect(page.getByText(/By enabling bank sync, you will be granting/)).toBeVisible();
