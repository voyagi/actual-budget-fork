...
provides:
  ...
  - sync-server healthcheck enabling service_healthy dependency ordering
...
key-decisions:
  ...
  - "sync-server ports section removed (commented out) - Caddy and cloudflared handle all ingress over Docker network"
...
- Added sync-server healthcheck (upstream pattern) enabling `service_healthy` dependency ordering
...
- `docker-compose.yml` - 3-service stack with healthcheck, actual_net network, caddy_data/caddy_config volumes
...
- sync-server ports section commented out (not deleted) — preserves intent documentation; production never exposes port directly to host
...
None. The CLOUDFLARE_TUNNEL_TOKEN warning from `docker compose config --quiet` is expected — the token is intentionally unset in version control and populated from `.env` at runtime.

## User Setup Required

**External services require manual configuration before `docker compose up` works fully.**
...
- Phase 5 verification (Plans 02+) can now run `docker compose up` to start the 3-service stack
- Cloudflare Tunnel token must be configured in `.env` before cloudflared starts successfully
- Caddy local CA trust on desktop is a one-time step: `docker compose exec caddy cat /data/caddy/pki/authorities/local/root.crt > caddy-root.crt`
